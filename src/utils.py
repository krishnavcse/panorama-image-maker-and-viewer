...
def load_images(source_dir):
    ...
    image_list = [print(cv2.imread(os.path.join(source_dir, img), 1)) for img in imgs]
...
def parse(source_dir):
    ...
    #print(filenames) #fllenames consists of images list as a list
    #print(focal_length) #focial_length consists of focal length for every image as a list
    rep = source_dir.replace("/", "*/") #to go to parent directory E.g: cd ..
    data = rep.split("*")
    data.pop()
    source_d = ''.join(data)
    img_list = [cv2.imread(os.path.join(source_d, f), 1) for f in filenames]
    i_list = [os.path.join(source_d, f) for f in filenames]
